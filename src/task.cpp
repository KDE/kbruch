											...
											short padd_sub, short pmul_div)
{
	...
	} while (high(2, max_product_length) > pmax_md); 
...
ratio task::solve()
{
	...
	do
	{
		...
		{
			case ADD :
			case SUB : switch(*(op_pointer - 1))
						  {
								/* we only have to add/sub the next ratio */
								case ADD : ergebnis = ergebnis + *ratio_pointer++;
											  break;
								case SUB : ergebnis = ergebnis - *ratio_pointer++;
											  break;
						  }
						  break;
			case MUL :
			case DIV : switch (*(op_pointer - 1))
						  {
								/* the next ratio is a product, so we have to
								 * compute this product first and than add/sub it */
								case ADD : ergebnis = ergebnis +
														product(ratio_pointer, op_pointer);
											  break;
								case SUB : ergebnis = ergebnis -
														product(ratio_pointer, op_pointer);
											  break;
						  }
						  break;
			 ...
			 ShortArray::iterator & op_pointer)
{
	...
	do
	{
		...
		{
			case ADD :
			case SUB : return product; /* finished */

			/* compute the next step of the product (or div) */
			case MUL : product = product * *ratio_pointer++;
						  ++op_pointer;
						  break;
			case DIV : product = product / *ratio_pointer++;
						  ++op_pointer;
						  break;
		}
	} while (op_pointer != op_vector.end());
...
unsigned int task::high(unsigned short base, unsigned short exponent)
{
